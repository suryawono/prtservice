    ...
    <div class="row">
        <div class="col-md-4">
            <div class="dataTables_info">
                <?php
                echo $this->Paginator->counter("Total data {:count}. Halaman {:page} dari {:pages} halaman. ");
                ?>
            </div>
        </div>
        <div class="col-md-8">
        ...
        <label>
            ...
            <select size="1" class="select2-offscreen" style="overflow:none" id="pagination-limit">
                ...
                foreach ($limits as $k => $v) {
                    ...
                    $options .= '<option data-url="' . Router::url("/{$url}", true) . "/limit:{$k}{$get}" . '" ' . $select . ' value="' . $k . '">' . $v . '</option>';
                }
                echo $options;
                ?>
            </select>
